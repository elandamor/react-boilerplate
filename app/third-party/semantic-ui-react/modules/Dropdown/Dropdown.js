...
export default class Dropdown extends Component {
      ...
      if (hasValue && nextProps.multiple && !isNextValueArray) {
        console.error(
          "Dropdown `value` must be an array when `multiple` is set." +
            ` Received type: \`${Object.prototype.toString.call(
              nextProps.value
            )}\`.`
        );
  ...
  handleItemClick = (e, item) => {
    ...
    const optionSize = _.size(this.getMenuOptions());
    if (!multiple || isAdditionItem || optionSize === 1) {
      this.clearSearchQuery();
    }
  ...
  handleSearchChange = (e, { value }) => {
    ...
    _.invoke(this.props, "onSearchChange", e, {
      ...this.props,
  ...
  getMenuOptions = (value = this.state.value, options = this.props.options) => {
    ...
    if (multiple) {
      filteredOptions = _.filter(
        filteredOptions,
        opt => !_.includes(value, opt.value)
      );
    }
  ...
  setSelectedIndex = (
    ...
    if (!selectedIndex || selectedIndex < 0) {
      ...
      newSelectedIndex = _.includes(enabledIndicies, activeIndex)
        ? activeIndex
        : undefined;
    }
  ...
  computeSearchInputWidth = () => {
    ...
    if (this.sizerRef && searchQuery) {
      ...
      const searchWidth = Math.ceil(
        this.sizerRef.getBoundingClientRect().width
      );
